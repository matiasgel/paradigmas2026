#!/usr/bin/env python3
"""
knowledge_space.py ÔÇö KST Engine: Knowledge Space Theory (S13.1)
================================================================
...
class KSTEngine:
    ...
    def frontier(self, known_concepts: set[str]) -> list[str]:
        ...
        conceptos que el estudiante A├ÜN NO conoce pero cuyos
    ...
    def next_concept(self, student_state: dict[str, float]) -> str | None:
        """
        Dado el estado del estudiante (concepto ÔåÆ mastery 0-1),
        concepto frontera con mayor utilidad pedag├│gica.
        ...
        if not candidates:
            return None
        # Ordenar por centralidad (m├ís descendientes = mayor prioridad)
    ...
    def learning_path(
        ...
        """
        Camino m├¡nimo de aprendizaje desde el estado actual
        ...
        # Ordenar topol├│gicamente (prerequisitos primero)
        path.reverse()
...
def main() -> int:
    ...
    if not config.get("knowledge_graph_enabled", False):
        print("Ôä╣´©Å  knowledge_graph_enabled no est├í habilitado. Saliendo.")
        return 0

    kg_path = project_root / "salida" / "cursadas" / args.course / "knowledge-graph.json"
    if not kg_path.exists():
        print(f"ÔØî KG no encontrado: {kg_path}. Ejecutar 'knowledge_graph.py build' primero.")
        return 1
    ...
    if args.command == "frontier":
        ...
        print(f"­ƒôÜ Frontera de aprendizaje ({len(front)} conceptos):")
        for c in front:
            print(f"   ÔÇó {c}")
        ...
        if nxt:
            ...
            print(f"­ƒÄ» Siguiente concepto recomendado: {nxt} (desbloquea {desc} conceptos)")
        else:
            print("­ƒÄô ┬íTodos los conceptos dominados!")

    elif args.command == "path":
        if not args.target:
            print("ÔØî --target requerido para el comando 'path'")
            return 1
        path = engine.learning_path(args.target, state)
        print(f"­ƒù║´©Å  Camino hacia '{args.target}' ({len(path)} pasos):")
        for i, c in enumerate(path, 1):
            marker = "­ƒÄ»" if c == args.target else "­ƒôû"
            print(f"   {i}. {marker} {c}")
