#!/usr/bin/env python3
"""
edu_director.py ÔÇö Pipeline Orchestrator Minimalista (S12.1)
============================================================
Orquesta todo el pipeline de producci├│n de un tema con un solo comando,
usando checkpoints persistentes para reanudaci├│n.
    ...
    *,
    ...
    if dry_run:
        return StepResult(
            name=script_name,
            status="skipped",
            message=f"[dry-run] Se ejecutar├¡a: python {script_name} {' '.join(args)}",
        )
...
def _human_gate(prompt_msg: str, *, skip_gates: bool = False) -> StepResult:
    ...
    try:
        resp = input(f"\n­ƒöÆ {prompt_msg} [Enter para continuar / 'n' para cancelar]: ").strip()
        if resp.lower() == "n":
...
def run_pipeline(
    ...
    *,
    ...
    if resume and state_path.exists():
        ...
        print(f"­ƒöä Reanudando pipeline desde paso {start_step + 1}")
    ...
    print(f"\n­ƒÜÇ EDU Pipeline ÔÇö {topic} ({course})")
    ...
    for i, (step_name, step_fn) in enumerate(steps):
        ...
        result = step_fn()
        print(f"ÔåÆ {result.status}" + (f" ({result.duration_s}s)" if result.duration_s else ""))
        ...
        if result.status == "failed":
            print(f"\nÔØî Pipeline detenido en paso '{step_name}': {result.message}")
            print(f"   Reanudar con: python scripts/edu_director.py --resume --topic {topic} --course {course}")
            return 1

    state.completed = True
    state.save(state_path)
    print(f"\nÔ£à Pipeline completado exitosamente.")
    return 0
