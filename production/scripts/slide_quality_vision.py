#!/usr/bin/env python3
"""
Slide Quality Vision ÔÇö CLIP score + layout metrics (S10.3).

Eval├║a calidad visual de slides usando:
- OpenCLIP: relevancia imagen-texto, coherencia inter-slide, detecci├│n de clipart
- Layout: balance, whitespace, alineaci├│n a grilla
    ...
    for i, slide in enumerate(slides, 1):
        ...
        if not elements:
            results.append({
                ...
                "balance": 0.5,
                "grade": "B",
                "notes": "Sin elementos EMU ÔÇö an├ílisis limitado",
            })
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Slide Quality Vision ÔÇö CLIP + Layout")
    ...
    if not topic_dir.exists():
        print(f"ÔÜá´©Å Directorio del tema no encontrado: {topic_dir}")
        sys.exit(0)
    ...
    if model is None:
        print("ÔÜá´©Å OpenCLIP no disponible. An├ílisis visual limitado a layout.")
        print("  Para habilitar: pip install open-clip-torch")
    elif not thumbnails_dir.exists():
        print(f"ÔÜá´©Å Thumbnails no encontrados en {thumbnails_dir}")
        ...
        for thumb in sorted(thumbnails_dir.glob("*.png")):
            ...
            label = "Ô£à" if score > 0.25 else ("ÔÜá´©Å" if score > 0.15 else "ÔØî")
    ...
    # Generate report
    report_lines = [
        f"# Visual Quality Report ÔÇö {args.topic}\n",
        f"**Curso:** {args.course}\n",
    ]
    ...
    print(f"Ô£à Visual quality report: {report_path}")

    # Summary
    if layout_results:
        grades = [r["grade"] for r in layout_results]
        good = grades.count("A") + grades.count("B")
        print(f"­ƒôè Layout: {good}/{len(grades)} slides con grade A/B")
    if clip_results:
        relevant = sum(1 for r in clip_results if r["clip_score"] > 0.25)
        print(f"­ƒôè CLIP: {relevant}/{len(clip_results)} im├ígenes relevantes al texto")
