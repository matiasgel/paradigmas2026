#!/usr/bin/env python3
"""
knowledge_graph.py ÔÇö Knowledge Graph Engine: Ontolog├¡a Educativa Formal (S11.1)
================================================================================
Construye y valida un grafo de conocimiento educativo en formato JSON-LD,
con validaciones SPARQL sobre una ontolog├¡a OWL Lite.
...
Opcionales: owlready2, SPARQLWrapper (para validaci├│n externa)
"""
...
class CourseKnowledgeGraph:
    """Grafo de conocimiento educativo con soporte JSON-LD y validaci├│n."""
    ...
    def find_orphans(self, plan_concepts: set[str]) -> set[str]:
        """Conceptos en el plan que no est├ín en el grafo."""
    ...
    def check_bloom_monotonic(self) -> list[str]:
        ...
        for e in self.edges:
            ...
            if prereq_bloom and target_bloom:
                ...
                if p > t:
                    warnings.append(
                        f"Bloom descendente: {e['from']}({prereq_bloom}) ÔåÆ {e['to']}({target_bloom})"
                    )
        return warnings
    ...
    def validate(self) -> Result[dict]:
        ...
        if cycles:
            for c in cycles:
                errors.append(f"Ciclo detectado: {' ÔåÆ '.join(c)}")
        report = {
...
def build_kg_from_plan(project_root: Path, course: str) -> Result[CourseKnowledgeGraph]:
    """Construye KG a partir del plan m├¡nimo del curso."""
...
def main() -> int:
    ...
    if not config.get("knowledge_graph_enabled", False):
        print("Ôä╣´©Å  knowledge_graph_enabled no est├í habilitado en config. Saliendo.")
        return 0
    ...
    if args.command == "build":
        ...
        if not result.is_ok:
            for e in result.errors:
                print(f"ÔØî {e}", file=sys.stderr)
            return 1
        kg = result.unwrap()
        kg.save(kg_path)
        print(f"Ô£à KG generado: {kg_path}")
        print(f"   Conceptos: {len(kg.concepts)} | Relaciones: {len(kg.edges)}")
        return 0

    elif args.command == "validate":
        if not kg_path.exists():
            print(f"ÔØî KG no encontrado: {kg_path}. Ejecutar 'build' primero.")
            return 1
        kg = CourseKnowledgeGraph.load(kg_path)
        result = kg.validate()
        if result.is_ok:
            report = result.unwrap()
            print(f"Ô£à KG v├ílido ÔÇö {report['total_concepts']} conceptos, {report['total_edges']} relaciones")
            if report["bloom_warnings"]:
                print("ÔÜá´©Å  Advertencias de Bloom:")
        ...
        print(f"Ô£à Mermaid generado: {viz_path}")
        return 0
