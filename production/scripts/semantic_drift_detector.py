#!/usr/bin/env python3
"""
semantic_drift_detector.py ÔÇö Detector de inconsistencias sem├ínticas (S9.1)

Compara definiciones entre clases para detectar drift de vocabulario,
inconsistencias y saltos tem├íticos abruptos. Reutiliza MiniLM ya
...
Exit codes:
    0 ÔÇö reporte generado (o sin material)
    1 ÔÇö error
"""
...
# Patrones para extraer definiciones expl├¡citas
DEFINITION_PATTERNS = [
    ...
    re.compile(r"(?:\*\*|__)(\w[\w\s]{2,30})(?:\*\*|__)\s*[:ÔÇöÔÇô-]\s*(.{10,200}?)(?:\.|$)", re.MULTILINE),
]


def extract_definitions(text: str) -> list[dict]:
    """Extrae definiciones expl├¡citas del texto."""
    defs = []
...
def analyze_definitions(all_defs: dict[str, list[dict]], embedder) -> list[dict]:
    ...
    for term, occurrences in term_map.items():
        ...
        if embedder is not None:
            ...
            # Sin embedder, solo reportar definiciones m├║ltiples
            ...
            issues.append({
                ...
                "term": term,
                "topics": topics,
                "note": "Instalar sentence-transformers para an├ílisis de similitud",
            })
...
def generate_report(def_issues: list[dict], coherence_issues: list[dict], output_path: Path, course_id: str) -> str:
    ...
    lines = [
        f"# An├ílisis de Consistencia Sem├íntica ÔÇö {course_id}",
        f"\n> Generado: {now}\n",
    ]
    ...
    if def_issues:
        lines.extend([
            "## Definiciones\n",
            "| Severidad | T├®rmino | Tema A | Tema B | Similitud |",
            "|-----------|---------|--------|--------|-----------|",
        ])
        ...
        lines.append("\nÔ£à Sin inconsistencias detectadas en definiciones.\n")
    ...
    if coherence_issues:
        ...
        lines.append("\nÔ£à Coherencia narrativa entre temas sin saltos abruptos.\n")
...
def main() -> int:
    ...
    if not topics_dir.is_dir():
        print(f"Ôä╣´©Å  No hay temas en {topics_dir}")
        return 0
    ...
    if not topics_text:
        print("Ôä╣´©Å  Sin minutas/filminas disponibles para analizar")
        return 0

    embedder = load_embedder()
    if embedder is None:
        print("ÔÜá´©Å  sentence-transformers no instalado ÔÇö an├ílisis limitado a pattern matching")
