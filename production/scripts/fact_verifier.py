#!/usr/bin/env python3
"""
fact_verifier.py ÔÇö Pipeline NLI de verificaci├│n factual (S9.3)

Descompone texto en claims at├│micos, busca evidencia en ChromaDB
y plan-minimo, y clasifica con NLI (cross-encoder) o fallback heur├¡stico.

Uso:
    python scripts/fact_verifier.py --topic 05-sorting --course leng-2026

Exit codes:
    0 ÔÇö todos los claims verificados o neutros
    1 ÔÇö al menos un claim refutado (CONTRADICTION)
"""
...
def extract_claims(text: str) -> list[str]:
    """Descompone texto en claims at├│micos (oraciones verificables)."""
    # Separar en oraciones
    sentences = re.split(r"(?<=[.!?])\s+", text)
    claims = []
    for s in sentences:
        s = s.strip()
        # Filtrar l├¡neas no verificables (encabezados, directivas, c├│digo)
...
def retrieve_evidence(claim: str, course_dir: Path) -> list[str]:
    ...
    if plan_path.exists():
        ...
        # Buscar p├írrafos relevantes por palabras clave
...
def classify_nli(claim: str, evidence: list[str]) -> dict:
    ...
    if not evidence:
        return {"verdict": "NEUTRAL", "confidence": 0.0, "label": "ÔÜá´©Å"}
    ...
    try:
        ...
        if best_score["entailment"] > 0.7:
            return {"verdict": "ENTAILMENT", "confidence": best_score["entailment"], "label": "Ô£à"}
        if best_score["contradiction"] > 0.7:
            return {"verdict": "CONTRADICTION", "confidence": best_score["contradiction"], "label": "ÔØî"}
        return {"verdict": "NEUTRAL", "confidence": max(best_score.values()), "label": "ÔÜá´©Å"}

    except ImportError:
        # Fallback heur├¡stico: word overlap
        claim_words = set(claim.lower().split())
        max_overlap = 0
        for ev in evidence:
        ...
        if max_overlap > 0.5:
            return {"verdict": "ENTAILMENT", "confidence": max_overlap, "label": "Ô£à"}
        if max_overlap < 0.1:
            return {"verdict": "NEUTRAL", "confidence": 1 - max_overlap, "label": "ÔÜá´©Å"}
        return {"verdict": "NEUTRAL", "confidence": 0.5, "label": "ÔÜá´©Å"}
...
def generate_report(results: list[dict], output_path: Path, topic_id: str) -> str:
    ...
    lines = [
        f"# Fact-Check Report ÔÇö {topic_id}",
        f"\n> Generado: {now}",
        f"> Claims analizados: {len(results)} | Ô£à {len(verified)} | ÔØî {len(contradictions)} | ÔÜá´©Å {len(neutral)}\n",
    ]

    if contradictions:
        lines.extend([
            "## ÔØî Claims refutados (requieren revisi├│n)\n",
            "| # | Claim | Confianza |",
            "|---|-------|-----------|",
        ])
...
def main() -> int:
    ...
    if not texts:
        print(f"Ôä╣´©Å  Sin textos para verificar en {topic_folder}")
        return 0

    full_text = "\n".join(texts)
    claims = extract_claims(full_text)
    if not claims:
        print("Ôä╣´©Å  Sin claims verificables extra├¡dos")
        return 0

    print(f"­ƒöì Verificando {len(claims)} claims...")
    ...
    print(f"Ô£à Reporte generado: {path}")

    if contradictions > 0:
        print(f"ÔØî {contradictions} claims refutados ÔÇö requieren revisi├│n humana")
        return 1
