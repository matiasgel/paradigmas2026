#!/usr/bin/env python3
"""
parse_filminas.py ÔÇö Genera plan DRAFT desde filminas.md (v3 ÔÇö Schema-Driven)
=============================================================================
Primer paso del flujo v3: filminas.md ÔåÆ plan-draft-{tema}.json

Lee TODOS los enums y mapeos desde schema-registry.json ÔÇö no importa
LAYOUT_MAP ni IMAGE_STRATEGY de slides_pipeline.py.

DIFERENCIA CLAVE con v2:
  - type: usa SOLO la directiva @tipo:. Sin @tipo: ÔåÆ type: "pending".
    NUNCA infiere tipos desde el t├¡tulo o contenido.
...
El agente (slides-designer o class-writer) recibe el DRAFT, asigna tipos expl├¡citos
...
Flujo completo v3:
    1. python scripts/parse_filminas.py <topic_folder>         ÔåÆ DRAFT JSON
    2. Agente completa tipos y prompts en plan-draft-{tema}.json
    3. Renombrar a plan-filminas-{tema}.json (o el agente lo hace)
    4. python scripts/repair_plan.py <topic_folder>            ÔåÆ valida y repara
    5. python scripts/slides_pipeline.py <topic_folder>        ÔåÆ publica

Requiere:
    _edu/slides-config.yaml                ÔÇö sistema de dise├▒o
    _edu/schemas/schema-registry.json      ÔÇö mapeos y enums
...
# Pipeline compartido ÔÇö utilidades centralizadas
...
def generate_draft(filminas_path: Path, _config: dict, template_id: str) -> tuple[dict, int, int]:
    ...
    draft = {
        ...
        "slides": plan_slides,
        # Instrucciones para el agente ÔÇö eliminar del plan-filminas final
        "_draft_instructions": {
            ...
            "next_steps": [
                "1. Leer schema-registry.json ANTES de cualquier edici├│n",
                "2. Asignar type expl├¡cito desde canonical_types a cada slide en pending_type_ids",
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="parse_filminas.py ÔÇö Plan DRAFT desde filminas.md (v3, schema-driven, JSON output)",
    ...
    if not config_path.exists():
        print("ÔØî Falta _edu/slides-config.yaml ÔÇö ejecutar /edu-slides-designer primero.")
        sys.exit(1)
    ...
    if not registry_path.exists():
        print("ÔÜá´©Å  _edu/schemas/schema-registry.json no encontrado ÔÇö usando fallback.")

    print(f"­ƒôï Parseando {filminas_path.relative_to(project_root)} ÔÇª")
    try:
        draft, n_pending_types, n_pending_prompts = generate_draft(filminas_path, config, template_id)
    except ValueError as exc:
        print(f"ÔØî Contrato de filminas inv├ílido:\n{exc}")
        sys.exit(1)
    ...
    print(f"""
Ô£à Plan DRAFT generado: {draft_path.relative_to(project_root)}
   {total} filminas en total
   {total - n_pending_types} con tipo expl├¡cito (@tipo: en filminas.md)
   {n_pending_types} con type: pending ÔÇö agente debe asignar tipo expl├¡cito
   {n_pending_prompts} sin prompt de imagen ÔÇö agente debe escribir prompt visual puro""")
