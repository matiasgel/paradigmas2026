#!/usr/bin/env python3
"""
generate_tts.py ÔÇö Generador de audio TTS desde minuta (S7.3)
...
Exit codes:
    0 ÔÇö audio generado (o feature desactivada)
...
def extract_slide_scripts(minuta_path: Path) -> list[dict]:
    """Extrae el gui├│n por filmina desde minuta.md."""
    ...
    for match in pattern.finditer(text):
        ...
        # Extraer texto ├║til como gui├│n (limpiar markdown)
...
async def generate_edge_tts(text: str, output_path: Path, voice: str, rate: str) -> bool:
    ...
    try:
        ...
        print("ÔØî edge-tts no instalado. Ejecutar: pip install edge-tts")
        return False
...
def main() -> int:
    ...
    if not config.get("tts_enabled", False):
        print("Ôä╣´©Å  TTS desactivado (tts_enabled: false en config.yaml)")
        return 0
    ...
    if not minuta_path.exists():
        print(f"ÔØî minuta.md no encontrada: {minuta_path}")
        return 1

    slides = extract_slide_scripts(minuta_path)
    if not slides:
        print("Ôä╣´©Å  No se encontraron guiones de filminas en minuta.md")
        return 0
    ...
    if provider == "edge-tts":
        print(f"­ƒöè Generando {len(slides)} audios con edge-tts (voz: {voice})...")
        for slide in slides:
            output = audio_dir / f"filmina-{slide['number']:02d}.mp3"
            success = asyncio.run(generate_edge_tts(slide["script"], output, voice, rate))
            if success:
                print(f"  Ô£à filmina-{slide['number']:02d}.mp3")
            else:
                return 1
    elif provider in ("gcloud-tts", "elevenlabs"):
        confirm = input(f"ÔÜá´©Å  {provider} es un servicio pago. ┬┐Continuar? [s/N] ")
        if confirm.lower() != "s":
            print("Cancelado.")
            return 0
        print(f"ÔØî Provider {provider} requiere implementaci├│n adicional.")
        return 1
    else:
        print(f"ÔØî Provider no soportado: {provider}")
        return 1
    ...
    print(f"Ô£à Manifiesto: {manifest_path}")
    print(f"Ô£à {len(slides)} audios generados en {audio_dir}")
    return 0
