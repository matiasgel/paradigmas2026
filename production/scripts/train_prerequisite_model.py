#!/usr/bin/env python3
"""
train_prerequisite_model.py ÔÇö Entrenamiento CPL con LectureBank (S11.2)
=======================================================================
Script auxiliar para entrenar el clasificador de prerequisitos.
Dise├▒ado para correr en Google Colab T4 (o CPU local).
...
def main() -> int:
    ...
    if args.dataset == "custom" and args.csv:
        ...
        if not csv_path.exists():
            print(f"ÔØî CSV no encontrado: {csv_path}")
            return 1
        pairs = load_custom_dataset(csv_path)
        print(f"­ƒôè Dataset custom: {len(pairs)} pares")
    else:
        pairs = SEED_PAIRS
        print(f"­ƒôè Dataset seed (LectureBank-inspired): {len(pairs)} pares")
    ...
    try:
        ...
        print("­ƒöº Usando XGBoost")
    except ImportError:
        from sklearn.ensemble import GradientBoostingClassifier

        model = GradientBoostingClassifier(n_estimators=100, max_depth=4)
        print("­ƒöº Usando GradientBoostingClassifier (sklearn fallback)")
    ...
    print(f"Ô£à Accuracy (CV): {scores.mean():.2f} ┬▒ {scores.std():.2f}")

    # Save
    model_dir.mkdir(parents=True, exist_ok=True)
    with model_path.open("wb") as f:
        pickle.dump({"model": model, "is_trained": True}, f)
    print(f"­ƒÆ¥ Modelo guardado: {model_path}")
