#!/usr/bin/env python3
"""
Captura thumbnails de una presentaci├│n Google Slides publicada.
...
def find_project_root(start: Path) -> Path:
    ...
    while True:
        ...
        if cur == cur.parent:
            break
        cur = cur.parent
    raise FileNotFoundError("No se pudo encontrar la ra├¡z del proyecto.")
...
def capture_thumbnails(presentation_id: str, output_dir: Path) -> None:
    ...
    print(f"  Descargando lista de slides de presentaci├│n {presentation_id}...")
    ...
    for i, slide in enumerate(slides):
        ...
        if resp.status_code != 200:
            print(f"  [{i+1:02d}/{len(slides)}] ÔØî Error thumbnail API: {resp.status_code} ÔÇö {resp.text[:100]}")
            failed.append(i + 1)
            continue
        ...
        for attempt in range(3):
            try:
                ...
                print(f"  [{i+1:02d}/{len(slides)}] ÔÜá´©Å  Intento {attempt+1}/3 timeout: {e}")
        ...
        if img_resp and img_resp.status_code == 200:
            ...
            ok += 1
            print(f"  [{i+1:02d}/{len(slides)}] Ô£à {out_path.name}")
        else:
            code = img_resp.status_code if img_resp else "timeout"
            print(f"  [{i+1:02d}/{len(slides)}] ÔØî Error descargando imagen: {code}")
            failed.append(i + 1)
