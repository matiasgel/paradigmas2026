#!/usr/bin/env python3
"""
EDU Knowledge Watcher ÔÇö Monitorea ingesta/ y auto-ingesta PDFs en ChromaDB.

Cuando se detecta un nuevo PDF en ingesta/:
  1. Convierte a TXT (pdfminer)
  2. Chunkea con tama├▒o 800 chars
...
def process_pdf(pdf_path: Path) -> None:
    ...
    if txt_path.exists():
        print(f"  ÔÅ¡´©Å  TXT ya existe: {txt_path.name}")
        return

    # Convert PDF ÔåÆ TXT
    n = convert_pdfs_to_txt(pdf_path.parent)
    if n == 0:
        print(f"  ÔÜá´©Å  No se pudo convertir: {pdf_path.name}")
        return

    # Ingest the new TXT
    if txt_path.exists():
        count = ingest_single_txt(txt_path)
        print(f"  Ô£à Ingestado: {txt_path.name} ÔåÆ {count} chunks")
...
class IngestaHandler(FileSystemEventHandler):
    ...
    def on_created(self, event: FileCreatedEvent) -> None:
        ...
        if path.suffix.lower() == ".pdf":
            print(f"\n­ƒôÑ Nuevo PDF detectado: {path.name}")
    ...
    def on_moved(self, event: FileMovedEvent) -> None:
        ...
        if path.suffix.lower() == ".pdf":
            print(f"\n­ƒôÑ PDF movido/renombrado: {path.name}")
            time.sleep(1)
            process_pdf(path)
...
def main() -> None:
    ...
    if not watch_path.exists():
        ...
        print(f"­ƒôü Creada carpeta: {watch_path}")

    print(f"­ƒæÇ Monitoreando: {watch_path}")
