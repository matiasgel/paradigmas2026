#!/usr/bin/env python3
"""
EDU Knowledge Base ÔÇö ChromaDB ingestion & query system.

Ingesta documentos de _edu-knowledge/ (referencias acad├®micas + docs de herramientas)
y material del curso de ingesta/ en ChromaDB. Si los PDFs de ingesta/ no tienen TXT
generados, los convierte autom├íticamente con pdfminer.
...
Chunk sizes:
    ...
    - material: max 800 chars (texto denso de libros ÔåÆ chunks peque├▒os para
      mejor retrieval sem├íntico)
"""
...
MATERIAL_CHUNK_SIZE = 800  # chars ÔÇö chunks peque├▒os para texto denso de libros
...
# ---------------------------------------------------------------------------
# PDF ÔåÆ TXT conversion (integrated)
# ---------------------------------------------------------------------------
...
def convert_pdfs_to_txt(folder: Path) -> int:
    ...
    try:
        ...
        print("  ÔÜá´©Å  pdfminer.six no instalado ÔÇö omitiendo conversi├│n PDFÔåÆTXT")
    ...
    for pdf in sorted(folder.rglob("*.pdf")):
        ...
        try:
            ...
            if not text or not text.strip():
                text = f"[PDF sin texto extra├¡ble: {pdf.name}]\n"
            ...
            converted += 1
            print(f"  ­ƒôä PDFÔåÆTXT: {pdf.name}")
        except Exception as exc:
            print(f"  ÔØî Error convirtiendo {pdf.name}: {exc}")
    return converted
...
# ---------------------------------------------------------------------------
# Ingestion ÔÇö material del curso
# ---------------------------------------------------------------------------
...
def chunk_ingesta_txt(text: str, source: str, book: str, max_chars: int = MATERIAL_CHUNK_SIZE) -> list[dict]:
    ...
    doubleÔÇænewline paragraph separators.
    """
...
def collect_material_documents() -> list[dict]:
    """Collect all TXT files from ingesta/**/txt/. AutoÔÇæconverts PDFs first."""
    all_chunks = []
    if not INGESTA_DIR.is_dir():
        print("  ÔÜá´©Å  Carpeta ingesta/ no encontrada, omitiendo material del curso.")
        return all_chunks

    n_converted = convert_pdfs_to_txt(INGESTA_DIR)
    if n_converted:
        print(f"  Ô£à Convertidos {n_converted} PDFs nuevos a TXT\n")
    ...
    for f in txt_files:
        ...
        if chunks:
            ...
            print(f"  ­ƒôû {source}: {len(chunks)} chunks  [{book[:50]}]")
    return all_chunks
...
def collect_documents(include_material: bool = False) -> list[dict]:
    ...
    if REFS_DIR.is_dir():
        for f in sorted(REFS_DIR.glob("*.md")):
            ...
            all_chunks.extend(chunks)
            print(f"  ­ƒôÜ {f.name}: {len(chunks)} chunks")
    ...
    if TOOLS_DIR.is_dir():
        for f in sorted(TOOLS_DIR.iterdir()):
            if f.suffix in (".md", ".yml", ".yaml"):
                ...
                all_chunks.extend(chunks)
                print(f"  ­ƒöº {f.name}: {len(chunks)} chunks")
                ...
                print(f"  ÔÅ¡´©Å  {f.name}: skipped (HTML, use markdown refs instead)")

    # Course material (ingesta/**/txt/*.txt)
    if include_material:
        print("\n­ƒôÑ Recolectando material del curso (ingesta/)...")
...
def ingest(force: bool = False, include_material: bool = False):
    ...
    if force:
        try:
            ...
            print("­ƒùæ´©Å  Collection eliminada (--force)")
        except Exception:
            pass
    ...
    if existing > 0 and not force:
        print(f"Ôä╣´©Å  Collection ya tiene {existing} documentos. Agregando nuevos sin borrar...")

    print("­ƒôÑ Recolectando documentos...")
    chunks = collect_documents(include_material=include_material)

    if not chunks:
        print("ÔÜá´©Å  No se encontraron documentos en _edu-knowledge/")
        return
    ...
    print(f"\nÔ£à Ingestados {len(chunks)} chunks de {len(set(c['source'] for c in chunks))} documentos")
...
def search(query: str, doc_type: str | None = None, n_results: int = 8):
    ...
    try:
        ...
        print("ÔÜá´©Å  Knowledge base vac├¡a. Ejecutar: python scripts/knowledge_base.py ingest")
        sys.exit(1)
    ...
    for doc, meta, dist in zip(
        ...
        relevance = 1 - dist  # cosine distance ÔåÆ similarity
        entries.append({
            "text": doc,
            "source": meta["source"],
            "type": meta["type"],
...
def print_results(entries: list[dict]):
    ...
    if not entries:
        return
    print(f"\n­ƒöì {len(entries)} resultados:\n")
    for i, e in enumerate(entries, 1):
        icon = "­ƒôÜ" if e["type"] == "reference" else "­ƒöº"
        print(f"{'ÔöÇ'*60}")
        print(f"{icon} [{i}] {e['source']} ÔÇö {e['heading']}")
...
def list_docs():
    ...
    try:
        ...
        print("ÔÜá´©Å  Knowledge base vac├¡a.")
        return

    count = collection.count()
    print(f"­ƒôè {count} chunks indexados\n")
    ...
    print(f"{'ÔöÇ'*55} {'ÔöÇ'*12} {'ÔöÇ'*6}")
    for src, info in sorted(sources.items()):
        icon = "­ƒôÜ" if info["type"] == "reference" else "­ƒöº"
...
def query_knowledge(query: str, doc_type: str | None = None, n_results: int = 5) -> str:
    ...
    Designed for agent consumption ÔÇö returns a single string.
    """
    ...
    parts = []
    for i, e in enumerate(entries, 1):
        parts.append(f"[{i}] {e['source']} ÔÇö {e['heading']} (relevancia: {e['relevance']})")
...
def main():
    parser = argparse.ArgumentParser(
        description="EDU Knowledge Base ÔÇö ChromaDB",
    ...
    p_search.add_argument("query", help="Texto de b├║squeda")
