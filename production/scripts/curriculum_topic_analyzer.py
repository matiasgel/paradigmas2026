#!/usr/bin/env python3
"""
curriculum_topic_analyzer.py ÔÇö BERTopic Curriculum Analyzer (S9.2)

Detecta gaps curriculares y redundancias comparando plan m├¡nimo
...
def extract_topics_from_plan(plan_path: Path) -> list[str]:
    """Extrae t├│picos del plan m├¡nimo."""
...
def generate_report(analysis: dict, output_path: Path, course_id: str) -> str:
    """Genera reporte Markdown de an├ílisis curricular."""
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    lines = [
        f"# An├ílisis Curricular ÔÇö {course_id}",
        f"\n> Generado: {now}\n",
    ]
    ...
    if gaps:
        lines.extend([
            "## ­ƒö┤ Gaps curriculares (t├│picos no cubiertos)\n",
            "| T├│pico del plan | Similitud m├íxima |",
            "|-----------------|------------------|",
        ])
        for g in gaps:
            lines.append(f"| {g['topic']} | {g['max_similarity']:.2f} |")
    else:
        lines.append("\nÔ£à Sin gaps ÔÇö todos los t├│picos del plan est├ín cubiertos.\n")
    ...
    if redundancies:
        lines.extend([
            "\n## ÔÜá´©Å Redundancias (>3 apariciones)\n",
...
def main() -> int:
    ...
    if not config.get("topic_analysis_enabled", False):
        print("Ôä╣´©Å  An├ílisis de t├│picos desactivado (topic_analysis_enabled: false)")
        return 0
    ...
    if not plan_topics:
        print("Ôä╣´©Å  plan-minimo.md no encontrado o sin t├│picos")
        return 0
    ...
    if not theme_concepts:
        print("Ôä╣´©Å  Sin artefactos de temas disponibles")
        return 0
    ...
    try:
        ...
        print("ÔÜá´©Å  sentence-transformers no instalado ÔÇö usando matching por palabras clave")
