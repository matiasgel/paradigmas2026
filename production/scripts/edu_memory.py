#!/usr/bin/env python3
"""EDU Collective Memory ÔÇö SQLite FTS5 backend.
...
def export_memory(
    *,
    ...
    # Markdown
    lines = [f"# Memoria Colectiva ÔÇö {course_id or 'todas las clases'}\n"]
    ...
    for cat in sorted(by_cat):
        lines.append(f"\n## {cat}\n")
        for e in by_cat[cat]:
            status = "Ô£à" if e["resolved"] else "ÔÅ│"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="EDU Collective Memory ÔÇö SQLite FTS5",
...
def _print_entries(entries: list[dict]) -> None:
    if not entries:
        ...
        return
    for e in entries:
        status = "Ô£à" if e.get("resolved") else "ÔÅ│"
        topic = f"T{e['topic_num']}" if e.get("topic_num") else "ÔÇö"
        agent_s = e.get("agent") or "ÔÇö"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.command == "resolve":
        if resolve_entry(args.id):
            print(f"Ô£à Entry #{args.id} marked as resolved.")
            return 0
        print(f"ÔØî Entry #{args.id} not found.")
        return 1
