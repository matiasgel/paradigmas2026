#!/usr/bin/env python3
"""
Bloom Classifier ÔÇö Clasificador DeBERTa fine-tuned para taxonom├¡a de Bloom (S10.1).
...
# Keywords heur├¡sticos por nivel de Bloom (fallback sin modelo ML)
BLOOM_KEYWORDS: dict[str, list[str]] = {
    "recordar": ["definir", "listar", "nombrar", "identificar", "enumerar", "qu├® es", "cu├íl es", "mencionar"],
    "comprender": ["explicar", "describir", "resumir", "interpretar", "comparar", "diferenciar", "por qu├®"],
    "aplicar": ["aplicar", "implementar", "resolver", "calcular", "usar", "demostrar", "programar", "codificar"],
    "analizar": ["analizar", "descomponer", "clasificar", "examinar", "distinguir", "relaci├│n entre", "causa"],
    "evaluar": ["evaluar", "justificar", "argumentar", "criticar", "juzgar", "defender", "cu├íl es mejor"],
    "crear": ["dise├▒ar", "crear", "proponer", "desarrollar", "construir", "inventar", "formular", "planificar"],
}


def _classify_keyword_fallback(question: str) -> tuple[str, float]:
    """Clasificador heur├¡stico basado en keywords. Fallback sin modelo ML."""
...
def main():
    parser = argparse.ArgumentParser(description="Bloom Classifier ÔÇö Taxonom├¡a de Bloom por pregunta")
    parser.add_argument("--course", help="ID del curso (ej: leng-2026)")
    parser.add_argument("--exam", help="Nombre del examen (ej: parcial-1)")
    parser.add_argument("--file", help="Archivo con preguntas (una por l├¡nea)")
    ...
    if args.file:
        ...
        if not file_path.exists():
            print(f"ÔØî Archivo no encontrado: {args.file}")
            sys.exit(1)
        ...
        if not exam_file.exists():
            print(f"ÔÜá´©Å No se encontr├│ blueprint para {args.exam} en {eval_dir}")
    ...
    if not questions:
        print("ÔÜá´©Å No se encontraron preguntas para clasificar.")
        sys.exit(0)

    if not model_path.exists():
        print("ÔÜá´©Å Modelo DeBERTa fine-tuned no encontrado. Usando fallback por keywords.")
    ...
    # Output
    print(f"\n­ƒôè Clasificaci├│n Bloom ÔÇö {len(results)} preguntas\n")
    print(f"{'#':<4} {'Nivel':<12} {'Conf.':<8} {'M├®todo':<10} {'Pregunta':<60}")
    print("-" * 94)
