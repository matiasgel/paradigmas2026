#!/usr/bin/env python3
"""
generate_exam_blueprint.py ÔÇö Generador de Blueprints de Examen (S2.2)

Genera una tabla de especificaciones (blueprint) para parciales con:
- Distribuci├│n ponderada de temas seg├║n tiempo dedicado en clase
- Distribuci├│n por niveles de Bloom (RecordarÔåÆEvaluar)
- Validaci├│n contra exam-blueprint.schema.json
...
def estimate_topic_weight(topic_id: str, topic_folder: Path) -> float:
    ...
    Heur├¡stica: cuenta filminas si existe filminas.md, o usa peso uniforme.
    """
...
def blueprint_to_markdown(blueprint: dict) -> str:
    ...
    lines = [
        f"# Blueprint de Examen ÔÇö {blueprint['course_id']}",
    ...
    for level, pct in blueprint["bloom_profile_used"].items():
        bar = "Ôûê" * round(pct * 20) + "Ôûæ" * (20 - round(pct * 20))
    ...
    lines.extend([
        ...
        "- **Aplicar:** Resolver ejercicios, escribir c├│digo, usar f├│rmulas",
...
def main() -> None:
    ...
    parser.add_argument("--time", type=int, default=120,
                        help="Duraci├│n del examen en minutos (default: 120)")
    parser.add_argument("--bloom-profile", default="default",
                        choices=list(BLOOM_PROFILES.keys()),
                        help="Perfil de distribuci├│n Bloom (default: default)")
    ...
    if schema_path.exists():
        try:
            ...
            print("Ô£à Blueprint v├ílido contra exam-blueprint.schema.json")
        except ImportError:
            print("ÔÜá´©Å jsonschema no instalado ÔÇö validaci├│n de schema omitida")
        except Exception as e:
            print(f"ÔÜá´©Å Blueprint no pasa validaci├│n de schema: {e}")

    print(f"Ô£à Blueprint generado:")
