...
class SlidesContractTests(unittest.TestCase):
    def test_parse_filminas_returns_pending_without_directive(self) -> None:
        """v2 (Sprint 2): slides sin @tipo: expl├¡cito deben tener type='pending', no inferido."""
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Slide sin @tipo: ÔÇö antes se inferia como concepto-abstracto, ahora debe ser pending
            filminas.write_text(
                "### [F-00] Introduccion\n\n# Bienvenida\n\n- Punto A\n- Punto B\n",
                encoding="utf-8",
            )
            ...
            # v2: sin @tipo: ÔåÆ type debe ser 'pending', nunca inferido del contenido
    ...
    def test_invalid_directive_fails_fast(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "@tipo inv├ílido"):
