#!/usr/bin/env python3
"""
edu_smolagent_director.py ÔÇö Director Agent con smolagents (S12.2)
=================================================================
Orquestador inteligente que usa LLMs (Qwen/Llama v├¡a HF Inference API)
para decidir qu├® hacer cuando pasos fallan o hay feedback inesperado.
...
Requisitos:
    ...
    - HF_TOKEN en variables de entorno (gratuito v├¡a HuggingFace Inference API)
...
def _make_tool_functions() -> dict:
    ...
    def run_slides_pipeline(topic: str, course: str) -> str:
        """Ejecuta el pipeline de generaci├│n de slides."""
        result = subprocess.run(
...
def run_smolagent_director(topic: str, course: str, config: dict) -> int:
    ...
    try:
        ...
        print("ÔØî smolagents no instalado. Ejecute: pip install smolagents")
        print("   Cayendo a modo minimal...")
        return _fallback_minimal(topic, course)

    hf_token = os.environ.get("HF_TOKEN")
    if not hf_token:
        print("ÔÜá´©Å  HF_TOKEN no configurado. Usando modo minimal como fallback.")
        return _fallback_minimal(topic, course)
    ...
    prompt = f"""Eres un director de producci├│n acad├®mica. Tu objetivo es producir
...
Ejecuta cada paso y reporta el resultado."""
    ...
    try:
        ...
        print(f"\nÔ£à Director Agent complet├│: {result}")
        return 0
    except Exception as e:
        print(f"ÔØî Error en smolagent: {e}")
        return 1
...
def main() -> int:
    ...
    if not config.get("orchestrator_enabled", False):
        print("Ôä╣´©Å  orchestrator_enabled no est├í habilitado en config. Saliendo.")
        return 0

    mode = config.get("orchestrator_mode", "minimal")
    if mode == "minimal":
        print("Ôä╣´©Å  Modo minimal ÔÇö ejecutando edu_director.py")
