#!/usr/bin/env python3
"""
generate_interactive.py ÔÇö Generador de HTML interactivo desde spec (S7.1)
...
Tipos soportados (v1):
  - sorting-visualizer: comparaci├│n de algoritmos de sorting
  - tree-explorer: visualizaci├│n de BST/AVL
  - stack-simulator: simulaci├│n de stack push/pop/peek
  - fsm-simulator: aut├│mata finito interactivo
  - memory-layout: visualizaci├│n stack/heap/data
  - custom: template vac├¡o

Uso:
    python scripts/generate_interactive.py --spec interactivos/spec.json --topic 05-sorting --course leng-2026

Exit codes:
    0 ÔÇö generado (o feature desactivada)
...
<div class="canvas-area" role="application" aria-label="{title}">
  <canvas id="sim" width="760" height="360" tabindex="0" 
          aria-label="Simulaci├│n interactiva de {concept}"></canvas>
</div>
<div class="controls" role="toolbar" aria-label="Controles de simulaci├│n">
  {controls}
</div>
<p class="info">Tipo: {sim_type} ┬À Bloom: {bloom_level} ┬À Teclado: Tab + Enter</p>
<script>
{script}
</script>
</body>
...
def generate_sorting_visualizer(spec: dict) -> tuple[str, str]:
    ...
    controls = """
<button onclick="reset()" aria-label="Reiniciar">­ƒöä Reset</button>
<button onclick="step()" aria-label="Paso">ÔÅ¡ Paso</button>
<button onclick="play()" aria-label="Reproducir">ÔûÂ Play</button>
...
def generate_stack_simulator(spec: dict) -> tuple[str, str]:
    ...
    controls = """
<button onclick="push_val()" aria-label="Push">Ô¼å Push</button>
<button onclick="pop_val()" aria-label="Pop">Ô¼ç Pop</button>
<button onclick="peek_val()" aria-label="Peek">­ƒæü Peek</button>
<button onclick="reset()" aria-label="Reset">­ƒöä Reset</button>"""
...
function pop_val() {
  ...
  const v = stack.pop(); msg = 'pop() ÔåÆ ' + v; draw();
}
function peek_val() {
  msg = stack.length ? 'peek() ÔåÆ ' + stack[stack.length-1] : 'Stack vac├¡o';
  draw();
}
...
def generate_generic(spec: dict) -> tuple[str, str]:
    """Genera un template gen├®rico para tipos no espec├¡ficos."""
    controls = '<button onclick="draw()" aria-label="Ejecutar">ÔûÂ Ejecutar</button>'
    script = """
const c = document.getElementById('sim');
const ctx = c.getContext('2d');
function draw() {
  ...
  ctx.fillText('Simulaci├│n de """ + spec.get("type", "custom") + """', 200, 180);
  ctx.fillStyle = '#aaa'; ctx.font = '14px system-ui';
...
def generate_html(spec: dict) -> str:
    ...
    return HTML_TEMPLATE.format(
        title=spec.get("title", "Simulaci├│n EDU"),
...
def main() -> int:
    ...
    if not config.get("interactive_scenes_enabled", False):
        print("Ôä╣´©Å  Simulaciones interactivas desactivadas (interactive_scenes_enabled: false)")
        return 0
    ...
    if not spec_path.exists():
        print(f"ÔØî Spec no encontrada: {spec_path}")
        return 1

    spec = load_json(spec_path)
    sim_type = spec.get("type", "custom")
    if sim_type not in SUPPORTED_TYPES:
        print(f"ÔØî Tipo no soportado: {sim_type}. V├ílidos: {', '.join(sorted(SUPPORTED_TYPES))}")
        return 1
    ...
    if size_kb > max_kb:
        print(f"ÔÜá´©Å  HTML generado ({size_kb:.0f}KB) excede el l├¡mite ({max_kb}KB)")
    ...
    print(f"Ô£à Simulaci├│n generada: {output_path} ({size_kb:.0f}KB)")
    return 0
