#!/usr/bin/env python3
"""
validate_slide_composition.py ÔÇö Auditor├¡a visual de composici├│n de slides (S1.2)

Valida que las filminas respeten buenas pr├ícticas de composici├│n visual:
- M├írgenes seguros (5% del borde)
- Densidad visual (35-55% ideal, Scheiter & Eitel)
- Detecci├│n de superposiciones entre bounding boxes
...
# GEOMETR├ìA (le├¡da de pipeline-runtime.schema.json)
# ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ


def load_canvas_geometry(project_root: Path) -> dict:
    """Carga geometr├¡a de canvas del pipeline-runtime schema."""
...
# ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
# VALIDACIONES DE COMPOSICI├ôN
# ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ


def check_safe_margins(slide: dict, canvas: dict) -> list[str]:
    """Verifica que los elementos est├®n dentro del ├írea segura (5% del borde).
    ...
    # El layout es metadata ÔÇö si el pipeline respeta el schema,
    # los m├írgenes est├ín controlados por pipeline-runtime.
    # Aqu├¡ verificamos consistencia del layout declarado.
    layout = slide.get("layout", {})

    # Filminas con body en "full-center" o "full-bottom" necesitan
    # especial atenci├│n al margen inferior
    ...
    if body_zone in ("full-center", "full-bottom"):
        ...
        if total_text > 500:
            issues.append("Mucho texto en layout full ÔÇö riesgo de exceder margen inferior")
...
def estimate_visual_density(slide: dict) -> dict:
    ...
    Densidad ideal: 35-55% del ├írea total (Scheiter & Eitel, 2017).

    Heur├¡stica: cuenta "peso visual" de cada elemento:
    - T├¡tulo: 10% base
    - Body text: ~2% por cada 50 caracteres
    - Code block: 15% base + 1% por cada 10 l├¡neas
...
def grade_density(density_pct: float, slide_type: str) -> str:
    ...
    # pero eso es intencional ÔÇö no penalizar
    if slide_type in ("portada", "cierre", "socratica"):
        return "A"  # siempre OK por dise├▒o
...
def check_element_overlap(slide: dict) -> list[str]:
    ...
    for region, elements in active_regions.items():
        if len(elements) > 1:
            issues.append(
                f"Posible superposici├│n en regi├│n '{region}': {', '.join(elements)}"
            )
...
def generate_report(slide_analyses: list[dict], topic_id: str) -> str:
    """Genera reporte Markdown de composici├│n visual."""
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    lines = [
        f"# Reporte de Composici├│n Visual ÔÇö {topic_id}",
        f"**Generado:** {now}",
        f"**Basado en:** Scheiter & Eitel (2017), Patr├│n Z, Margen seguro 5%",
        "",
    ...
    for analysis in slide_analyses:
        ...
        emoji = {"A": "Ô£à", "B": "­ƒƒí", "C": "ÔÜá´©Å", "F": "ÔØî"}.get(grade, "?")
        if grade == "F":
            total_grade_f += 1

        obs = []
        if overlap_count > 0:
            obs.append(f"{overlap_count} superposici├│n(es)")
        if margin_count > 0:
            obs.append(f"{margin_count} margen(es)")
        obs_str = "; ".join(obs) if obs else "ÔÇö"
    ...
    if overlaps_found:
        ...
        for a in overlaps_found:
            lines.append(f"### {a['slide_id']} ÔÇö {a['title']}")
            for o in a["overlaps"]:
                lines.append(f"- ÔÜá´©Å {o}")
            lines.append("")
    ...
    lines.extend([
        ...
        f"**{'Ô£à PASA' if total_grade_f == 0 else 'ÔÜá´©Å REQUIERE ATENCI├ôN'}**",
    ])
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Auditor de composici├│n visual para filminas EDU"
    )
