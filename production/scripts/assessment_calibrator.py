#!/usr/bin/env python3
"""
Assessment Calibrator ÔÇö IRT 2PL + BKT por concepto (S10.2).

Calibra dificultad real de ├¡tems de examen con IRT y estima
...
# ÔöÇÔöÇ BKT (Bayesian Knowledge Tracing) ÔÇö Python puro ÔöÇÔöÇ
...
# ÔöÇÔöÇ IRT 2PL (simplificado) ÔÇö Python puro ÔöÇÔöÇ
...
def estimate_irt_parameters(response_matrix: list[list[int]]) -> list[dict]:
    """
    Estimaci├│n simplificada de par├ímetros IRT 2PL.
    response_matrix: filas=alumnos, columnas=├¡tems (1=correcto, 0=incorrecto)
    """
    ...
    for j in range(n_items):
        ...
        # Difficulty: inversi├│n logit de p_value
        p_clamped = max(0.01, min(0.99, p_value))
        difficulty = -math.log(p_clamped / (1 - p_clamped))
        # Discrimination: correlaci├│n punto-biserial simplificada
...
def load_gradebook(csv_path: Path) -> tuple[list[str], list[str], list[list[int]]]:
    ...
    with open(csv_path, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            for val in row[1:]:
                val = val.strip()
                if val in ("1", "1.0", "true", "si", "s├¡", "correct"):
                    responses.append(1)
...
def main():
    parser = argparse.ArgumentParser(description="Assessment Calibrator ÔÇö IRT + BKT")
    parser.add_argument("--course", required=True, help="ID del curso")
    parser.add_argument("--gradebook", help="CSV de respuestas (alumnos ├ù ├¡tems)")
    ...
    if args.gradebook:
        ...
        if not csv_path.exists():
            print(f"ÔØî CSV no encontrado: {args.gradebook}")
            sys.exit(1)

        students, items, matrix = load_gradebook(csv_path)
        print(f"­ƒôè Gradebook: {len(students)} alumnos ├ù {len(items)} ├¡tems")
        ...
        # Report
        report_lines = [
            f"# IRT Report ÔÇö {args.course}\n",
            f"**Alumnos:** {len(students)} | **├ìtems:** {len(items)}\n",
            "| # | ├ìtem | Dificultad | Discriminaci├│n | p-valor | Flag |",
            "|---|------|------------|----------------|---------|------|",
        ]
        items_to_revise = []
        for r in irt_results:
            ...
            if r["discrimination"] < 0.2:
                flags.append("ÔÜá´©Å Baja discriminaci├│n")
            if r["difficulty"] > 2.5:
                flags.append("ÔØî Muy dif├¡cil")
            if r["difficulty"] < -2.5:
                flags.append("ÔÜá´©Å Trivial")
            flag_str = " / ".join(flags) if flags else "Ô£à"
        ...
        print(f"Ô£à IRT report: {output_dir / 'irt-report.md'}")

        if items_to_revise:
            revise = "# ├ìtems a Revisar\n\n"
            ...
            print(f"ÔÜá´©Å {len(items_to_revise)} ├¡tems flaggeados para revisi├│n")

        # BKT
        if args.bkt or True:  # Always run BKT when gradebook is provided
            bkt = BKTTracker()
            mastery_report = [
                f"# BKT Mastery ÔÇö {args.course}\n",
            ...
            for i, student in enumerate(students):
                for j, item_name in enumerate(items):
                    if matrix[i][j] >= 0:
                        ...
                        estado = "Ô£à Dominado" if p > 0.95 else ("ÔÜá´©Å En progreso" if p > 0.5 else "ÔØî Repaso urgente")
            ...
            print(f"Ô£à BKT mastery: {output_dir / 'bkt-mastery.md'}")
        ...
        with open(template, "w", encoding="utf-8", newline="") as f:
            ...
            writer.writerow(["Garc├¡a, M.", "1", "0", "1"])
            writer.writerow(["L├│pez, J.", "1", "1", "0"])
        print(f"­ƒôØ Template CSV generado: {template}")
