#!/usr/bin/env python3
"""
student_analytics.py ÔÇö Motor de analytics + importadores CSV (S5.1)

Importa notas de GitHub Classroom y Moodle, detecta alumnos en riesgo
con alertas sem├íforo, y genera dashboards Markdown.
...
def import_grades_csv(conn: sqlite3.Connection, csv_path: Path, course_id: str) -> int:
    ...
    if not csv_path.exists():
        print(f"ÔØî Archivo CSV no encontrado: {csv_path}")
        return 0
...
def compute_risk_alerts(conn: sqlite3.Connection, course_id: str) -> list[dict]:
    ...
    for student_id, avg_pct in rows:
        ...
        reason = f"Promedio {avg_pct:.0f}% ÔÇö por debajo del umbral m├¡nimo"
        action = "Contactar al alumno para tutor├¡a personalizada" if risk == "alto" else "Monitorear pr├│ximas entregas"
        alerts.append({
    ...
    for student_id, zeros in rows:
        reason = f"{zeros} entregas con score 0 ÔÇö posible abandono"
        alerts.append({
            "student_id": student_id, "risk_level": "alto",
            "reason": reason, "action": "Verificar situaci├│n del alumno",
        })
        conn.execute(
            "INSERT INTO risk_alerts (student_id, course_id, alert_date, risk_level, reason, suggested_action) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (student_id, course_id, now, "alto", reason, "Verificar situaci├│n del alumno"),
        )
...
def generate_dashboard(conn: sqlite3.Connection, course_id: str, output_path: Path) -> str:
    """Genera dashboard Markdown con estad├¡sticas y alertas."""
    ...
    ranges = conn.execute("""
        SELECT
            CASE
                WHEN score * 100.0 / max_score >= 80 THEN '­ƒƒó 80-100%'
                WHEN score * 100.0 / max_score >= 60 THEN '­ƒƒí 60-79%'
                WHEN score * 100.0 / max_score >= 40 THEN '­ƒƒá 40-59%'
                ELSE '­ƒö┤ 0-39%'
            END as rango,
            COUNT(*) as n
        FROM grades WHERE course_id = ?
    ...
    lines = [
        f"# Dashboard Analytics ÔÇö {course_id}",
        f"\n> Generado: {now}\n",
        "## Resumen\n",
        f"| M├®trica | Valor |",
        f"|---------|-------|",
    ...
    if alerts:
        lines.extend([
            "",
            "## ÔÜá´©Å Alertas de riesgo\n",
            "| Alumno | Nivel | Raz├│n | Acci├│n sugerida |",
            "|--------|-------|-------|-----------------|",
        ])
        for name, level, reason, action in alerts:
            emoji = "­ƒö┤" if level == "alto" else "­ƒƒí"
...
def main() -> int:
    ...
    try:
        if args.command == "import-grades":
            ...
            if count == 0:
                print("Ôä╣´©Å  No se importaron registros.")
                return 0
            print(f"Ô£à {count} registros importados para {args.course}")
            alerts = compute_risk_alerts(conn, args.course)
            if alerts:
                print(f"ÔÜá´©Å  {len(alerts)} alertas de riesgo generadas")
            return 0
            ...
            print(f"Ô£à Dashboard generado: {path}")
            return 0
    finally:
        conn.close()
