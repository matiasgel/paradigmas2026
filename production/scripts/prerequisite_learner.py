#!/usr/bin/env python3
"""
prerequisite_learner.py ÔÇö Concept Prerequisite Learning con ML (S11.2)
======================================================================
Classifier XGBoost/LightGBM para inferir relaciones de prerequisito
entre conceptos usando features sem├ínticas y estructurales.
...
def compute_features(
    ...
    # Feature 1: Similaridad sem├íntica
...
def run_active_learning(
    ...
    """Sesi├│n de active learning: presenta pares inciertos al docente."""
    pairs = list(combinations(concepts, 2))
    if not pairs:
    ...
    # Si el modelo ya est├í entrenado, seleccionar los m├ís inciertos
    ...
    annotations = []
    print(f"\n­ƒôØ Sesi├│n de anotaci├│n ÔÇö {len(indices)} preguntas")
    print("Responda S (s├¡) / N (no) / Q (salir)\n")
    ...
    if annotations and annotations_path:
        ...
        print(f"\nÔ£à {len(annotations)} anotaciones guardadas en {annotations_path}")
...
def main() -> int:
    ...
    if args.command == "predict":
        if not learner.is_trained:
            print("ÔÜá´©Å  Modelo CPL no entrenado. Ejecute 'annotate' primero o proporcione modelo.")
        ...
        print(f"Ô£à {len(suggestions)} prerequisitos sugeridos ÔåÆ {out_path}")
        for s in suggestions[:10]:
            print(f"   {s['from']} ÔåÆ {s['to']} (conf: {s['confidence']:.2f})")
