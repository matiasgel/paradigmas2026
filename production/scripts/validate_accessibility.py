#!/usr/bin/env python3
"""
validate_accessibility.py ÔÇö Validador de accesibilidad WCAG para filminas (S1.1)

Valida que las filminas cumplan criterios de accesibilidad:
- Contraste de colores (WCAG AA 4.5:1, AAA 7:1)
- Tama├▒o tipogr├ífico m├¡nimo seg├║n distancia de aula
- Presencia de alt_text en filminas con im├ígenes
...
Exit codes:
    ...
    1 ÔÇö error de entrada (topic no encontrado, plan no existe)
"""
...
# WCAG CONTRAST CALCULATION (WCAG 2.1 ┬º1.4.3 / ┬º1.4.6)
# ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
...
def _relative_luminance(r: int, g: int, b: int) -> float:
    """Calcula luminancia relativa WCAG 2.1 (f├│rmula sRGB linearizada)."""
...
def grade_contrast(ratio: float, large_text: bool = False) -> str:
    """Clasifica el contraste seg├║n WCAG 2.1."""
    if large_text:
        if ratio >= 4.5:
            return "AAA"
        if ratio >= 3.0:
...
# ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
# TAMA├æO TIPOGR├üFICO (distancia de aula)
# ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ


def min_font_size_pt(distance_meters: float, resolution: str = "1080p") -> float:
    """Tama├▒o m├¡nimo de fuente en pt para que el texto sea legible a distancia.

    Base: regla de 1/150 del ancho de proyecci├│n (ISO 3864-1 adaptada).
    Un proyector Full HD a 6m con pantalla de 2m ancho ÔåÆ ~0.013m por car├ícter
    ÔåÆ ~37pt m├¡nimo para texto de cuerpo.
    """
    # Ancho t├¡pico de proyecci├│n seg├║n distancia (throw ratio ~1.5:1)
    screen_width_m = distance_meters / 1.5
    # Resoluci├│n horizontal
    h_pixels = {"720p": 1280, "1080p": 1920, "4k": 3840}.get(resolution, 1920)
    # Un car├ícter debe ocupar al menos 1/100 del ancho de pantalla
    char_width_m = screen_width_m / 100
    # Convertir metros de car├ícter a puntos (1pt = 0.000352778m)
...
def validate_palette_contrast(slides_config: dict) -> list[dict]:
    """Valida contrastes entre pares de colores del sistema de dise├▒o."""
    ...
    pairs = [
        ...
        ("primary", "background", True),   # t├¡tulos = texto grande
        ("secondary", "primary", True),     # subt├¡tulos sobre fondo primario
...
def validate_slides_alt_text(slides: list[dict]) -> list[dict]:
    """Verifica que filminas con imagen tengan alt_text no vac├¡o."""
    issues = []
    for slide in slides:
...
def validate_font_sizes(slides_config: dict, distance_m: float,
    ...
    """Verifica que los tama├▒os de fuente sean legibles a la distancia dada."""
    ...
    for zone_name, zone_cfg in typography.items():
        ...
        if size and size < min_size:
            issues.append({
                ...
                "issue": f"Fuente {zone_name} ({size}pt) menor que m├¡nimo recomendado ({round(min_size, 1)}pt) a {distance_m}m",
            })
...
def generate_report(contrast_results: list[dict],
    ...
    lines = [
        f"# Reporte de Accesibilidad ÔÇö {topic_id}",
        f"**Generado:** {now}",
        f"**Est├índar:** WCAG 2.1 Level AA",
        "",
    ...
    for r in contrast_results:
        emoji = "Ô£à" if r["grade"] != "F" else "ÔØî"
        if r["grade"] == "F":
            all_pass = False
        lines.append(
            f"| {r['pair']} | `{r['fg']}` | `{r['bg']}` | "
            f"{r['ratio']}:1 | {emoji} {r['grade']} | "
            f"{'S├¡' if r['large_text'] else 'No'} |"
        )

    lines.extend(["", "## Alt Text en Im├ígenes", ""])
    if alt_issues:
        all_pass = False
        lines.append("| Slide | T├¡tulo | Problema |")
        lines.append("|-------|--------|----------|")
        for issue in alt_issues:
            lines.append(
                f"| {issue['slide_id']} | {issue['title']} | ÔØî {issue['issue']} |"
            )
    else:
        lines.append("Ô£à Todas las filminas con imagen tienen `alt_text`.")

    lines.extend(["", "## Tama├▒o Tipogr├ífico", ""])
    if font_issues:
        all_pass = False
        lines.append("| Zona | Actual | M├¡nimo | Distancia |")
        lines.append("|------|--------|--------|-----------|")
        ...
        lines.append("Ô£à Todos los tama├▒os de fuente cumplen el m├¡nimo recomendado.")

    lines.extend([
        "",
        "## Resultado Global",
        "",
        f"**{'Ô£à PASA' if all_pass else 'ÔÜá´©Å REQUIERE ATENCI├ôN'}**",
    ])
...
def main() -> None:
    ...
    if not edu_config.get("accessibility_check_enabled", False):
        print("Ôä╣´©Å  Accesibilidad desactivada. Activar con accessibility_check_enabled: true en config.yaml")
        sys.exit(0)
    ...
    if plan_result.is_ok:
        ...
        print(f"AVISO: No se encontr├│ plan JSON ÔÇö solo se validar├í paleta y tipograf├¡a.")

    # Validaciones
    contrast_results = validate_palette_contrast(slides_config)
    alt_issues = validate_slides_alt_text(slides)

    # Tama├▒o tipogr├ífico (solo si hay distancia configurada)
