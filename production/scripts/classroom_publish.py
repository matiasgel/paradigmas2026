#!/usr/bin/env python3
"""
classroom_publish.py ÔÇö Publicaci├│n de TPs a GitHub Classroom (S3.1)

Wrapper de `gh classroom` CLI para publicar assignments directamente.
Solo lectura de archivos EDU ÔÇö no modifica artefactos existentes.
...
Exit codes:
    0 ÔÇö publicado correctamente (o feature desactivada)
    1 ÔÇö error (gh CLI no instalado, topic no encontrado, etc.)
"""
...
def check_gh_auth() -> bool:
    """Verifica autenticaci├│n de gh CLI."""
    try:
...
def publish_assignment(
    ...
    title = f"{course_id} ÔÇö {topic_id}"
...
def main() -> int:
    ...
    if not edu_config.get("classroom_enabled", False):
        print("Ôä╣´©Å  GitHub Classroom desactivado (classroom_enabled: false en config.yaml)")
        return 0

    if not check_gh_cli():
        print("ÔØî GitHub CLI (gh) no est├í instalado.")
        print("   Instalaci├│n: https://cli.github.com/")
        print("   brew install gh  |  sudo apt install gh  |  winget install GitHub.cli")
        return 1

    if not check_gh_auth():
        print("ÔØî GitHub CLI no autenticado. Ejecutar: gh auth login")
        return 1
