...
12. **Scripts son INMUTABLES para agentes — salvo pedido explícito del docente** — Los archivos en `scripts/` (todos los `.py` y `requirements.txt`) son de SOLO LECTURA para todos los agentes EDU **por defecto**. Los agentes pueden LEER y EJECUTAR scripts, pero NUNCA crear, editar, renombrar ni borrar archivos en `scripts/` a menos que el docente lo solicite **de forma explícita** en el mensaje actual (p. ej.: "actualiza el script X", "corrige Y en scripts/", "sincroniza los scripts con production"). Ante cualquier solicitud de modificación, el agente debe listar exactamente qué archivos va a cambiar y confirmar con el docente antes de proceder. Si un script necesita una corrección o nueva funcionalidad que el docente no ha pedido explícitamente → escalar al docente para que lo apruebe. Para sincronizar scripts con la rama `production`: usar `git show origin/production:scripts/<archivo> | Out-File -FilePath scripts/<archivo>` o equivalente. Los scripts son el contrato técnico del pipeline — modificarlos sin aprobación del docente rompe la reproducibilidad.
