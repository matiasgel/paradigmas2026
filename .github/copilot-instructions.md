...
12. **Scripts en solo lectura por defecto (con excepcion explicita)** — Los archivos en `scripts/` (todos los `.py` y `requirements.txt`) se consideran de SOLO LECTURA para todos los agentes EDU. Los agentes pueden LEER y EJECUTAR scripts y, por defecto, NO deben crear, editar, renombrar ni borrar archivos en `scripts/`. **Excepcion:** si el usuario/docente lo solicita de forma explicita en la conversacion actual, el agente puede modificar scripts dentro del alcance pedido, manteniendo trazabilidad del cambio y sin tocar schemas ni templates. Esta regla aplica tambien a `scripts/` en `production/scripts/`.
