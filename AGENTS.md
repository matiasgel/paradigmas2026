...
11. **Knowledge Base ChromaDB** — `_edu-knowledge/` contiene referencias académicas (12 documentos) y documentación de herramientas (16 documentos + 6 fuentes OpenMAIC) ingestados en ChromaDB. La base de datos se guarda en **disco local fuera del repo** (`EDU_CHROMA_PATH` en `.env`, default: `~/.edu/chroma_db`) y NUNCA se commitea a git. Cada rama usa su propio directorio: rama `production` → `C:\Users\matia\Documents\chroma_db`. Los scripts cargan `.env` automáticamente sin depencias externas. Todos los agentes DEBEN consultar la knowledge base antes de implementar funcionalidades. Usar `python scripts/knowledge_base.py search "query"` para buscar. Filtrar por tipo: `--type reference` (papers académicos), `--type tool` (docs de herramientas) o `--type material` (libros del curso). Para ingestar: `/edu-ingest` (incremental por defecto, no destruye datos). Para backup: `/edu-backup`. Documentos disponibles: Multimedia Learning (Fiorella/Mayer 2023), Cognitive Load Theory (Sweller/Chen 2023), WCAG 2.2/3.0 (W3C), FSRS v4 (Ye 2023), Bloom Taxonomy & Assessment (Haladyna 2024), Learning Analytics (Ifenthaler/Tsai/Yan 2020-2024), CS Education & GitHub (SIGCSE/Feliciano/Denny 2023-2024), Slide Composition (Duarte/Scheiter 2019-2023), Adaptive Learning & ITS (VanLehn/ALEKS/Du 2023), MCP Protocol (Anthropic 2024-2025), MAIC (Yu et al. 2024, Tsinghua), OpenMAIC Platform (THU-MAIC 2026). API importable: `from scripts.knowledge_base import query_knowledge; result = query_knowledge("cognitive load slides")`

12. **Scripts modificables solo con petición y aprobación explícitas del usuario** — Por defecto, los archivos en `scripts/` (todos los `.py` y `requirements.txt`) y en `production/scripts/` son de SOLO LECTURA para los agentes EDU. Un agente puede crear, editar, renombrar o borrar estos archivos únicamente cuando el usuario haya solicitado explícitamente el cambio de código y haya aprobado explícitamente su alcance. Una petición clara que indique el cambio deseado y los scripts afectados cuenta como petición y aprobación; si el alcance es ambiguo, el agente DEBE pedir aprobación antes de editar. La autorización es válida solo para la tarea solicitada y no habilita modificaciones adicionales. Antes de modificar, el agente debe revisar el estado y diff existentes, preservar cambios ajenos y seguir las reglas de arquitectura del proyecto. Después de modificar, debe ejecutar verificaciones o tests pertinentes e informar los archivos cambiados y resultados. Sin autorización explícita, el agente solo puede LEER y EJECUTAR scripts y debe reportar o escalar la corrección necesaria. Los scripts siguen siendo el contrato técnico del pipeline y toda modificación debe quedar bajo control de versiones.
