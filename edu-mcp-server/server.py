...
Tools expuestos:
  ...
  - edu.validate_plan    : Validar un plan de filminas JSON contra el schema
...
def _find_project_root() -> Path:
    """Busca la raíz del proyecto subiendo hasta encontrar _edu/ o _edu-knowledge/."""
...
CHROMA_DIR = PROJECT_ROOT / "_edu-knowledge" / "chroma_db"
...
async def list_tools() -> list[Tool]:
    return [
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "course": {"type": "string", "description": "ID del curso (ej: iapjn-2026)"},
                },
        ...
        Tool(
            ...
            description="Buscar en la knowledge base ChromaDB de EDU (referencias academicas + herramientas).",
            inputSchema={
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "Texto a buscar"},
...
def _search_memory(query: str, limit: int) -> list[TextContent]:
    ...
    try:
        ...
        results = [f"[{cat}] ({ts})\n{content[:300]}" for cat, content, ts in rows]
        return [TextContent(type="text", text="\n---\n".join(results))]
    except sqlite3.OperationalError:
        return [TextContent(type="text", text="Tabla memory_entries no existe aun.")]
    finally:
        conn.close()


def _validate_plan(topic: str, course: str) -> list[TextContent]:
    script = SCRIPTS_DIR / "validate_plan.py"
...
def _search_knowledge(query: str, n_results: int, doc_type: str) -> list[TextContent]:
    script = SCRIPTS_DIR / "knowledge_base.py"
    if not script.exists():
