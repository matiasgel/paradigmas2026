...
:- consult('../_solucion_referencia').

% ============================================================
%  TEST DE VALIDACIÓN DE LA SOLUCIÓN DE REFERENCIA
%  (mismo contenido que test_ejercicios.pl pero contra _solucion_referencia.pl)
% ============================================================
...
test(destinos, [true(L == [parana, rosario, ushuaia])]) :- destinos_unicos(L).
...
test(par_normal, [true(Sorted == [2,4,6])]) :- pares([1,2,3,4,5,6], P), msort(P, Sorted).
test(par_ninguno, [true(P == [])])          :- pares([1,3,5], P).
test(par_vacia,   [true(P == [])])          :- pares([], P).
test(par_todos,   [true(Sorted == [2,4,6,8])]) :- pares([2,4,6,8], P), msort(P, Sorted).
...
test(color_distintos) :- colorear_triangulo(A, B, C), A \= B, A \= C, B \= C.
